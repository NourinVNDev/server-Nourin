...
export class managerVerifierRepository {
    ...
    async fetchSelectedVerifierRepository(verifierId:string){
        try {
    ...
    async postVerifierLoginRepository(formData:verifierFormData){
           const validEvents = formData.Events.filter(event => event.trim() !== "");
                ...
                async updateVerifierRepository(formData: verifierFormData) {
